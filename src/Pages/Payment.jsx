...
import {
  Wallet, CreditCard, ShieldCheck, ArrowLeft,
  ChevronRight, CheckCircle, Zap, Smartphone, Building2,
  Lock, Sparkles,
} from "lucide-react";
...
export default function Payment() {
  ...
  const handleCOD = async () => {
    ...
    try {
      const response = await API.post("/orders/create", {
        ...orderData,
        payment: { method: "COD", status: "pending", amount: orderData.total },
      });
  ...
  const handleOnlinePayment = async () => {
    ...
    try {
      const { data } = await API.post("/payment/create-order", { amount: orderData.total });
      ...
      const options = {
        ...
        modal: { ondismiss: () => { console.log("Payment cancelled"); setProcessingMethod(null); } },
        theme: { color: "#000" },
      };
    ...
    >
      ...
      >
        ...
        <div className="flex-1">
          ...
          <p className="text-[11px] text-gray-400 mt-0.5">Complete your order securely</p>
        </div>
        {/* Step indicator */}
        ...
        <div className="lg:flex lg:gap-8 lg:items-start">
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            <motion.div
              ...
              style={{
                background: "linear-gradient(135deg, #101010 0%, #1a1a1a 55%, #2a1f00 100%)",
                boxShadow: "0 8px 40px rgba(0,0,0,0.22)",
              }}
            >
              {/* Gold glow blobs */}
              <div className="absolute top-[-40px] right-[-40px] w-48 h-48 rounded-full pointer-events-none"
                style={{ background: "radial-gradient(circle, rgba(255,215,0,0.2), transparent 65%)" }} />
              <div className="absolute bottom-[-30px] left-[-20px] w-32 h-32 rounded-full pointer-events-none"
                style={{ background: "radial-gradient(circle, rgba(255,215,0,0.1), transparent 65%)" }} />
              ...
              <div className="relative z-10">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="font-black text-white mt-1.5"
                ...
                <div className="mt-5 pt-4 border-t border-white/10 grid grid-cols-3 gap-4">
                  {[
                    { label: "Items", val: `₹${orderData.originalTotal ?? orderData.total}` },
                    { label: "Delivery", val: orderData.deliveryFee === 0 ? "Free" : `₹${orderData.deliveryFee}` },
                    { label: "Handling", val: `₹${orderData.handlingFee ?? 15}` },
                  ].map((row) => (
                    <div key={row.label}>
            ...
            <motion.div
              ...
              style={{
                ...
                border: selectedMethod === "online"
                  ...
                  : "1.5px solid #EFEFEF",
                boxShadow: selectedMethod === "online"
            ...
            >
              ...
              <div className="p-5">
                <div className="flex items-start gap-4">
                  <div
                    ...
                    style={{
                      background: selectedMethod === "online" ? "rgba(255,215,0,0.15)" : "#F5F3FF",
                    }}
                  >
                    {processingMethod === "online" ? (
                      <motion.div
                        ...
                        style={{ borderColor: selectedMethod === "online" ? "#FFD700" : "#8B5CF6", borderTopColor: "transparent" }}
                      />
                    ) : (
                      <CreditCard size={20} color={selectedMethod === "online" ? "#FFD700" : "#8B5CF6"} />
                    )}
                  </div>
                  <div className="flex-1 min-w-0 pr-20">
                    <p className={`font-bold text-[15px] leading-none ${selectedMethod === "online" ? "text-white" : "text-gray-900"}`}>
                      Pay Online
                    </p>
                    <p className={`text-[12px] mt-1 ${selectedMethod === "online" ? "text-gray-400" : "text-gray-500"}`}>
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      {[
                        ...
                        <span
                          ...
                          style={{
                            background: selectedMethod === "online" ? "rgba(255,255,255,0.1)" : "#F3F4F6",
                            color: selectedMethod === "online" ? "rgba(255,255,255,0.7)" : "#6B7280",
                          }}
                        >
                          {m.icon} {m.label}
                        </span>
              ...
              <div
                ...
                style={{
                  borderColor: selectedMethod === "online" ? "rgba(255,255,255,0.08)" : "#F5F5F5",
                  background: selectedMethod === "online" ? "rgba(255,255,255,0.03)" : "#FAFAFA",
                }}
              >
                <span
                  className="text-[11px] font-medium flex items-center gap-1"
                  style={{ color: selectedMethod === "online" ? "rgba(255,255,255,0.4)" : "#9CA3AF" }}
                >
                  <Zap size={11} /> Instant confirmation
                </span>
                <div
                  className="w-7 h-7 rounded-full flex items-center justify-center"
                  style={{ background: selectedMethod === "online" ? "#FFD700" : "#F3F4F6" }}
                >
                  <ChevronRight size={14} color={selectedMethod === "online" ? "#101010" : "#9CA3AF"} />
                </div>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              style={{
                ...
                border: selectedMethod === "cod"
                  ? "1.5px solid rgba(255,215,0,0.4)"
                  : "1.5px solid #EFEFEF",
                boxShadow: selectedMethod === "cod"
                  ? "0 8px 32px rgba(0,0,0,0.22)"
                  : "0 2px 12px rgba(0,0,0,0.06)",
            ...
            >
              <div className="p-5">
                <div className="flex items-start gap-4">
                  <div
                    className="w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0"
                    style={{
                      background: selectedMethod === "cod" ? "rgba(255,215,0,0.15)" : "#FFF9E6",
                    }}
                  >
                    {processingMethod === "cod" ? (
                      ...
                      <Wallet size={20} color={selectedMethod === "cod" ? "#FFD700" : "#D97706"} />
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`font-bold text-[15px] leading-none ${selectedMethod === "cod" ? "text-white" : "text-gray-900"}`}>
                      Cash on Pickup
                    </p>
                    <p className={`text-[12px] mt-1 ${selectedMethod === "cod" ? "text-gray-400" : "text-gray-500"}`}>
                      Pay in cash when our team arrives
                    </p>
                    <div className="flex items-center gap-1.5 mt-3">
                      <CheckCircle size={12} color={selectedMethod === "cod" ? "#FFD700" : "#10B981"} />
                      <span
                        className="text-[11px] font-semibold"
                        style={{ color: selectedMethod === "cod" ? "rgba(255,255,255,0.6)" : "#065F46" }}
                      >
              ...
              <div
                className="px-5 py-3 flex items-center justify-between border-t"
                style={{
                  borderColor: selectedMethod === "cod" ? "rgba(255,255,255,0.08)" : "#F5F5F5",
                  background: selectedMethod === "cod" ? "rgba(255,255,255,0.03)" : "#FAFAFA",
                }}
              >
                <span
                  className="text-[11px] font-medium"
                  style={{ color: selectedMethod === "cod" ? "rgba(255,255,255,0.4)" : "#9CA3AF" }}
                >
                  Pay at doorstep
                </span>
                <div
                  className="w-7 h-7 rounded-full flex items-center justify-center"
                  style={{ background: selectedMethod === "cod" ? "#FFD700" : "#F3F4F6" }}
                >
                  <ChevronRight size={14} color={selectedMethod === "cod" ? "#101010" : "#9CA3AF"} />
                </div>
              </div>
            </motion.div>
          ...
          <div className="lg:w-[340px] flex-shrink-0 lg:sticky lg:top-24 mt-4 lg:mt-0">
            ...
            >
              ...
              {/* Items */}
              <div className="py-2 max-h-60 overflow-y-auto" style={{ scrollbarWidth: "thin" }}>
                {(orderData.items || []).map((item, i) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-base flex-shrink-0">{itemEmoji[item.name] || "🧺"}</span>
                      <div className="flex-1 min-w-0">
                ...
                <div className="px-5 py-3 border-t border-gray-100 space-y-2">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-[12px] text-gray-600 break-words leading-relaxed">
                      {orderData.address.fullAddress}, {orderData.address.city} – {orderData.address.pincode}
                    </p>
                  </div>
                  {orderData.pickup && (
              ...
              <div className="px-5 py-3 border-t border-gray-100 space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-gray-800">₹{orderData.originalTotal ?? orderData.total}</span>
                </div>
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className={`font-semibold ${orderData.deliveryFee === 0 ? "text-green-600" : "text-gray-800"}`}>
                    {orderData.deliveryFee === 0 ? "FREE" : `₹${orderData.deliveryFee}`}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Handling</span>
                  <span className="font-semibold text-gray-800">₹{orderData.handlingFee ?? 15}</span>
                </div>
              </div>
              ...
              <div className="px-5 pb-5">
                ...
                >
                  <div>
                    ...
                    <p className="font-black text-gray-900 text-2xl leading-none">₹{orderData.total}</p>
                  </div>
                  <div
                ...
                <div className="grid grid-cols-2 gap-3 mt-4">
                  <motion.button
                    ...
                    style={{ background: "linear-gradient(135deg,#101010,#1e1700)", color: "white", opacity: isLoading && processingMethod !== "online" ? 0.5 : 1 }}
                  >
                    {processingMethod === "online" ? (
                  ...
                  <motion.button
                    ...
                    disabled={isLoading}
                    className="py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-1.5"
                    style={{ background: "#FFF9E6", color: "#D97706", border: "1px solid #FDE68A", opacity: isLoading && processingMethod !== "cod" ? 0.5 : 1 }}
                  >
                    {processingMethod === "cod" ? (
                      <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 0.9, ease: "linear" }}
                ...
                <div className="flex items-center justify-center gap-1.5 mt-4">
                  ...
                  <span className="text-[10px] text-gray-400 font-medium">Secured by Razorpay</span>
                </div>
              </div>
            </motion.div>
          </div>
          ...
          >
            ...
            >
              <div className="text-left">
                ...
                <p className="text-white font-black text-xl leading-none">₹{orderData.total}</p>
              </div>
              <div className="flex items-center gap-2">
                {processingMethod === "online" ? (
                  <motion.div
